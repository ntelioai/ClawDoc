...
(function () {
  ...
  function loadUniverAssets() {
    if (_univerCoreLoad) return _univerCoreLoad;
    const base = '/app/vendor/univer/';
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = base + 'univer-preset-sheets-core.css';
    document.head.appendChild(link);
    // UMD bundles read globals set by earlier scripts, so order matters.
    _univerCoreLoad = (async () => {
      await loadScriptOnce(base + 'react.production.min.js');
      await loadScriptOnce(base + 'react-dom.production.min.js');
      await loadScriptOnce(base + 'rxjs.umd.min.js');
      await loadScriptOnce(base + 'univer-presets.umd.js');
      await loadScriptOnce(base + 'univer-preset-sheets-core.umd.js');
      await loadScriptOnce(base + 'univer-preset-sheets-core.en-US.js');
    })();
    return _univerCoreLoad;
  }

  function loadSheetJs() {
    if (!_sheetJsLoad) _sheetJsLoad = loadScriptOnce('/app/vendor/univer/xlsx.full.min.js');
    return _sheetJsLoad;
  }
  ...
  async function renderSpreadsheet(doc, viewer, bust) {
    ...
    try {
      ...
      const LocaleType = (window.UniverCore && window.UniverCore.LocaleType) || window.UniverPresets.LocaleType;
      const mergeLocales = (window.UniverCore && window.UniverCore.mergeLocales) || ((x) => x);
