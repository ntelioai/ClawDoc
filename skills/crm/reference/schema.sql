-- Canonical CRM schema. Idempotent (every object uses IF NOT EXISTS), so it is
-- safe to run against an existing ~/crm.sqlite. Kept byte-identical to the
-- SCHEMA_SQL block in ClawDoc/crm.js so both initialisers agree.
