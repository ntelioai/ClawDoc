...
### Fixed
- Claude panel: approving a blocked tool now actually unblocks it. `--allowedTools` is read by the CLI when it starts, so granting a tool to a session whose `claude` process was already running changed nothing — the next turn came back denied again, which is why "Allow" appeared to do nothing while "Allow & retry" worked. The grant now recycles the connection (deferred if a turn is in flight) so the next turn spawns with the updated allowlist, resumed from the same session id. Granted tools are also persisted with the tab, instead of being silently revoked by a reload.
