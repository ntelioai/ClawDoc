...
// monospace text, or a graceful "no preview" card for everything else).
...
const TEXT_EXTS = new Set([
  '.txt', '.text', '.csv', '.tsv', '.json', '.jsonc', '.yaml', '.yml', '.xml',
...
function walk(dir, ignores, docs, folders, root) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      // Only markdown/html/text carry searchable body text. Images, PDFs and
      // unknown binaries are indexed for the tree but never read as utf8.
...
function main() {
  ...
  const s = index.stats;
  console.log(`clawdoc: indexed ${docs.length} docs (${s.md} md, ${s.html} html, ${s.pdf} pdf, ${s.image} image, ${s.text} text, ${s.binary} other) across ${folders.size} folders in ${s.durationMs}ms`);
